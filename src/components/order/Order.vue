            ...
            <el-form :model="addressForm" :rules="addressFormRules" ref="addressFormRef" label-width="80px">
                <el-form-item label="省市区/县">
                   <el-cascader clearable change-on-select
                        ...
                        :options="cityList" :props="cityProps"
                        ></el-cascader>    
                </el-form-item>
...
export default {
  ...
  methods:{
    ...
    async showProgressBox(){
        const {data:res} = await this.$http.get('/kudidi',{id:5960798321390})
        if(res.data.meta.status !== 200){ this.$message.error('获取物流信息失败')}
        this.processInfo = res.data.data.data;
    ...
    async getOrderList(){
       const {data:res} =await this.$http.get('/orders',this.queryInfo)
     ...
     async getCityList(){
        const {data:res} = await this.$http.post('/citypicker')
